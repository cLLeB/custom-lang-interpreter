...
impl Interpreter {
    ...
    pub fn set_test_mode(&mut self, v: bool) {
        self.test_mode = v;
    }
    pub fn test_results(&self) -> (usize, usize) {
        (self.test_passed, self.test_failed)
    }
    pub fn set_profile_mode(&mut self, v: bool) {
        self.profile_mode = v;
    }
        ...
        match stmt {
            ...
            Stmt::For {
                ...
                let result = loop {
                    ...
                    match self.exec_stmt(body) {
                        ...
                        Ok(Signal::Continue)
                        | Ok(Signal::ContinueLabel(_))
                        | Ok(Signal::None)
                        | Ok(Signal::ExprValue(_)) => {}
                    }
                    ...
                    Value::Generator(g) => {
                        ...
                        loop {
                            let item = {
                                ...
                                let x = gs.values[gs.index].clone();
                                gs.index += 1;
                                x
                            };
                            v.push(item);
            ...
            Stmt::LetDestructObject { fields, init, pos } => {
                ...
                let obj = match &val {
                    ...
                    _ => {
                        return Err(CustomLangError::type_err(format!(
                            "object destructuring requires object, got {}",
                            val.type_name()
                        ))
                        .with_pos(pos))
                    }
            ...
            Stmt::Labeled { label, body, .. } => match self.exec_stmt(body)? {
                ...
                if let Stmt::Function { name: fn_name, .. } = target.as_ref() {
                    if let Some(decorator_fn) = Env::get(&self.env, name) {
                        if let Some(fn_val) = Env::get(&self.env, fn_name) {
                            let pos = Position::default();
                            let decorated =
                                self.call_value(decorator_fn, vec![fn_val], None, &pos)?;
    ...
    pub fn eval_expr(&mut self, expr: &Expr) -> Result<Value> {
        match expr {
            ...
            Expr::PropAssign {
                ...
                match &obj_val {
                    Value::Class(cls) => {
                        ...
                        Err(CustomLangError::type_err(format!(
                            "cannot set static field '{}' on class",
                            prop
                        ))
                        .with_pos(pos))
                    }
                ...
                match callee.as_ref() {
                    Expr::Prop { object, name, .. } => {
                        ...
                        if let Expr::Super { .. } = object.as_ref() {
                            let super_cls =
                                Env::get(&self.env, "__super_class__").ok_or_else(|| {
                                    CustomLangError::runtime("'super' used outside class method")
                                        .with_pos(pos)
                                })?;
                            let this_val = Env::get(&self.env, "this").ok_or_else(|| {
                                CustomLangError::runtime("'super' used outside class method")
                                    .with_pos(pos)
                            })?;
                                ...
                                let method = cls
                                    ...
                                    .ok_or_else(|| {
                                        CustomLangError::runtime(format!(
                                            "super class '{}' has no method '{name}'",
                                            cls.name
                                        ))
                                        .with_pos(pos)
                                    })?;
                            ...
                            return Err(
                                CustomLangError::runtime("'super' is not a class").with_pos(pos)
                            );
                        }
                        ...
                        if let Value::Generator(_) = &receiver {
                            ...
                            return self.call_builtin(
                                match method_name {
                                    "next" => "gen_next",
                                    "to_array" => "gen_to_array",
                                    _ => {
                                        return Err(CustomLangError::runtime(format!(
                                            "generator has no method '{method_name}'"
                                        ))
                                        .with_pos(pos))
                                    }
                                },
            ...
            Expr::Spread { pos, .. } => Err(CustomLangError::runtime(
                "spread operator used in invalid context",
            )
            .with_pos(pos)),
            ...
            Expr::Super { pos } => {
                ...
                Env::get(&self.env, "__super_class__").ok_or_else(|| {
                    CustomLangError::runtime("'super' used outside class method").with_pos(pos)
                })
            }
        ...
        match op {
            ...
            BinaryOp::BitwiseAnd => {
                if let (Value::Number(a), Value::Number(b)) = (l, r) {
                    ...
                    Err(self.type_err_binop("&", l, r, pos))
                }
            }
            BinaryOp::BitwiseOr => {
                if let (Value::Number(a), Value::Number(b)) = (l, r) {
            ...
            BinaryOp::NullCoalesce => Ok(if !matches!(l, Value::Null) {
                l.clone()
            } else {
                r.clone()
            }),
            BinaryOp::In => match r {
                ...
                _ => Err(
                    CustomLangError::type_err("'in' requires object or array on right side")
                        .with_pos(pos),
                ),
            },
            BinaryOp::Instanceof => match r {
                ...
                Value::Str(s) => Ok(Value::Bool(
                    l.type_name() == s.as_str() || l.is_instance_of(s),
                )),
    ...
    fn op_add(&self, l: &Value, r: &Value, pos: &Position) -> Result<Value> {
        match (l, r) {
            ...
            (Value::Str(a), other) if matches!(l, Value::Str(_)) => {
    ...
    fn get_property(&mut self, obj: &Value, name: &str, pos: &Position) -> Result<Value> {
        match obj {
            ...
            Value::Class(cls) => {
                if let Some(v) = cls.static_fields.get(name) {
                    return Ok(v.clone());
                }
                if let Some(m) = cls.static_methods.get(name) {
                    return Ok(Value::Function(Rc::clone(m)));
                }
                Ok(Value::Null)
            }
    ...
    fn get_method(&self, obj: &Value, name: &str, pos: &Position) -> Result<Value> {
        match obj {
            ...
            Value::Class(cls) => {
                ...
                Err(CustomLangError::runtime(format!(
                    "class '{}' has no static method '{name}'",
                    cls.name
                ))
                .with_pos(pos))
            }
        ...
        match func {
            ...
            Value::Builtin(name) => {
                let final_args = if let Some(t) = this {
                    let needs_this = name.starts_with("coll_")
            ...
            Value::Object(ref obj) => {
                ...
                if flags.0 {
                    ...
                    let fns = {
                        ...
                        b.get("__fns__")
                            .cloned()
                            .unwrap_or(Value::make_array(vec![]))
                    };
                ...
                if flags.1 {
                    ...
                    let cached = {
                        let b = obj.borrow();
                        if let Some(Value::Object(c)) = b.get("__cache__") {
                            c.borrow().get(&cache_key).cloned()
                        } else {
                            None
                        }
                    };
                ...
                if flags.2 {
                    ...
                    let (fn_val, stored_args) = {
                        let b = obj.borrow();
                        let f = b.get("__fn__").cloned().unwrap_or(Value::Null);
                        let stored = if let Some(Value::Array(a)) = b.get("__args__") {
                            a.borrow().clone()
                        } else {
                            vec![]
                        };
                    ...
                    let arity = if let Value::Function(ref fd) = fn_val {
                        fd.params.len()
                    } else {
                        args.len()
                    };
                ...
                if flags.3 {
                    ...
                    let (fn_val, bound) = {
                        let b = obj.borrow();
                        let f = b.get("__fn__").cloned().unwrap_or(Value::Null);
                        let bound = if let Some(Value::Array(a)) = b.get("__args__") {
                            a.borrow().clone()
                        } else {
                            vec![]
                        };
    ...
    fn call_fn(
            ...
            if let Value::Instance(ref inst) = t {
                if let Some(ref super_cls) = inst.borrow().class.superclass {
                    Env::define(
                        &fn_env,
                        "__super_class__",
                        Value::Class(Rc::clone(super_cls)),
                    );
                }
            }
        ...
        let positional_count = if named_obj.is_some() {
        ...
        if fd.is_generator {
            ...
            self.env = outer;
            result?;
            let gen = Rc::new(RefCell::new(GeneratorState {
                values: yielded,
                index: 0,
                done: false,
            }));
    ...
    fn collect_generator_yields(&mut self, body: &Stmt, yielded: &mut Vec<Value>) -> Result<()> {
        match body {
            ...
            Stmt::If {
                ...
                if self.eval_expr(cond)?.is_truthy() {
                    self.collect_generator_yields(then_b, yielded)?;
                } else if let Some(e) = else_b {
                    self.collect_generator_yields(e, yielded)?;
                }
            }
            Stmt::Return { value, .. } => {
                if let Some(e) = value {
                ...
                Stmt::Decorator {
                    ...
                    } = target.as_ref()
                    {
                        ...
                        if let Value::Function(dfd) = decorated {
                            if *is_static {
                                static_method_map.insert(mn.clone(), dfd);
                            } else {
                                method_map.insert(mn.clone(), dfd);
                            }
                        }
                    }
                }
    ...
    fn instantiate(&mut self, class_val: Value, args: Vec<Value>, pos: &Position) -> Result<Value> {
        ...
        let field_fns: Vec<(String, Rc<FnData>)> = cls
            .methods
            .iter()
            .filter(|(n, _)| n.starts_with("__field_") && n.ends_with("__"))
            .map(|(n, fd)| {
                (
                    n.trim_start_matches("__field_")
                        .trim_end_matches("__")
                        .to_string(),
                    Rc::clone(fd),
                )
    ...
    fn get_std_module(&self, path: &str) -> Result<HashMap<String, Value>> {
        match path {
            ...
            _ => Err(CustomLangError::runtime(format!(
                "unknown standard module '{path}'"
            ))),
        }
    }
    ...
    fn exec_import(&mut self, path: &str, alias: Option<&str>) -> Result<()> {
        ...
        if self.loading_modules.contains(&file_path) {
            return Err(CustomLangError::runtime(format!(
                "circular import detected: '{file_path}'"
            )));
        }
    ...
    fn exec_std_import(&mut self, path: &str, alias: Option<&str>) -> Result<()> {
        let mod_name = alias
            .unwrap_or_else(|| path.trim_start_matches("std/"))
            .to_string();
        let ns = self.get_std_module(path)?;
        Env::define(&self.env, &mod_name, Value::make_object(ns));
        Ok(())
    ...
    fn make_json_module() -> HashMap<String, Value> {
        ...
        m.insert(
            "is_valid".to_string(),
            Value::Builtin("json_is_valid".to_string()),
        );
        m
    }
    ...
    fn make_math_module() -> HashMap<String, Value> {
        ...
        m.insert(
            "clamp".to_string(),
            Value::Builtin("math_clamp".to_string()),
        );
        m.insert("sign".to_string(), Value::Builtin("math_sign".to_string()));
        m.insert(
            "hypot".to_string(),
            Value::Builtin("math_hypot".to_string()),
        );
        m.insert("gcd".to_string(), Value::Builtin("math_gcd".to_string()));
        m.insert("lcm".to_string(), Value::Builtin("math_lcm".to_string()));
        m.insert(
            "factorial".to_string(),
            Value::Builtin("math_factorial".to_string()),
        );
        m.insert(
            "is_nan".to_string(),
            Value::Builtin("math_is_nan".to_string()),
        );
        m.insert(
            "is_finite".to_string(),
            Value::Builtin("math_is_finite".to_string()),
        );
        m.insert(
            "random".to_string(),
            Value::Builtin("random_float".to_string()),
        );
        m.insert(
        ...
        m.insert(
            "degrees".to_string(),
            Value::Builtin("math_degrees".to_string()),
        );
        m.insert(
            "radians".to_string(),
            Value::Builtin("math_radians".to_string()),
        );
        m.insert(
            "atan2".to_string(),
            Value::Builtin("math_atan2".to_string()),
        );
        ...
        m.insert(
            "log10".to_string(),
            Value::Builtin("math_log10".to_string()),
        );
        m.insert("cbrt".to_string(), Value::Builtin("math_cbrt".to_string()));
        m.insert(
            "trunc".to_string(),
            Value::Builtin("math_trunc".to_string()),
        );
        m
    }
    ...
    fn make_fs_module() -> HashMap<String, Value> {
        ...
        for (k, v) in [
            ...
            ("append_text", "fs_append_text"),
            ("exists", "fs_exists"),
            ("delete", "fs_delete"),
            ("rename", "fs_rename"),
            ("copy", "fs_copy"),
            ("mkdir", "fs_mkdir"),
            ("mkdir_all", "fs_mkdir_all"),
            ("rmdir", "fs_rmdir"),
            ...
            ("temp_file", "fs_temp_file"),
            ("temp_dir", "fs_temp_dir"),
            ("read_bytes", "fs_read_text"),
            ("write_bytes", "fs_write_text"),
        ] {
            m.insert(k.to_string(), Value::Builtin(v.to_string()));
        }
        m
    ...
    fn make_process_module() -> HashMap<String, Value> {
        ...
        for (k, v) in [
            ("args", "proc_args"),
            ("env", "proc_env"),
            ("env_all", "proc_env_all"),
            ("cwd", "proc_cwd"),
            ("chdir", "proc_chdir"),
            ("exit", "exit"),
            ("pid", "proc_pid"),
            ("platform", "proc_platform"),
            ("run", "proc_run"),
        ] {
            m.insert(k.to_string(), Value::Builtin(v.to_string()));
        }
    ...
    fn make_string_module() -> HashMap<String, Value> {
        ...
        for (k, v) in [
            ...
            ("at", "str_at"),
            ("index_of", "index_of"),
            ("last_index_of", "str_last_index_of"),
            ("char_codes", "str_char_codes"),
            ("from_char_codes", "str_from_char_codes"),
            ("is_digit", "str_is_digit"),
            ("is_alpha", "str_is_alpha"),
            ("is_alphanumeric", "str_is_alnum"),
            ("is_whitespace", "str_is_whitespace"),
            ("is_upper", "str_is_upper"),
            ("is_lower", "str_is_lower"),
            ("count_occurrences", "str_count_occurrences"),
            ("reverse", "str_reverse"),
    ...
    fn make_array_module() -> HashMap<String, Value> {
        ...
        for (k, v) in [
            ...
            ("take", "arr_take"),
            ("drop", "arr_drop"),
            ("take_while", "arr_take_while"),
            ("drop_while", "arr_drop_while"),
            ("flatten_deep", "arr_flatten_deep"),
            ("count", "count"),
            ("sum", "sum"),
            ("average", "average"),
            ("fill", "arr_fill"),
            ("fill_with", "arr_fill_with"),
            ("min_by", "arr_min_by"),
            ("max_by", "arr_max_by"),
            ("sort_by", "arr_sort_by"),
    ...
    fn make_object_module() -> HashMap<String, Value> {
        ...
        for (k, v) in [
            ("merge", "obj_merge"),
            ("deep_clone", "obj_deep_clone"),
            ("get_path", "obj_get_path"),
            ("set_path", "obj_set_path"),
            ("omit", "obj_omit"),
            ("pick", "obj_pick"),
            ("map_values", "obj_map_values"),
            ("map_keys", "obj_map_keys"),
            ("filter_values", "obj_filter_values"),
            ("invert", "obj_invert"),
            ("from_entries", "obj_from_entries"),
        ] {
            m.insert(k.to_string(), Value::Builtin(v.to_string()));
        }
        m
    ...
    fn make_datetime_module() -> HashMap<String, Value> {
        ...
        for (k, v) in [
            ("now", "dt_now"),
            ("from_timestamp", "dt_from_timestamp"),
            ("new", "dt_new"),
            ("format", "dt_format"),
            ("parse", "dt_parse"),
        ] {
    ...
    fn make_encoding_module() -> HashMap<String, Value> {
        let mut m = HashMap::new();
        for (k, v) in [
            ("url_encode", "enc_url_encode"),
            ("url_decode", "enc_url_decode"),
            ("html_encode", "enc_html_encode"),
            ("html_decode", "enc_html_decode"),
    ...
    fn make_testing_module() -> HashMap<String, Value> {
        ...
        for (k, v) in [
            ...
            ("assert_neq", "test_assert_neq"),
            ("assert_throws", "test_assert_throws"),
            ("assert_true", "test_assert_true"),
            ("assert_false", "test_assert_false"),
            ("before_each", "test_before_each"),
            ("after_each", "test_after_each"),
        ] {
            m.insert(k.to_string(), Value::Builtin(v.to_string()));
        }
        m
    ...
    fn make_weak_module() -> HashMap<String, Value> {
        ...
        m.insert(
            "WeakMap".to_string(),
            Value::Builtin("weakmap_new".to_string()),
        );
        m.insert(
            "WeakRef".to_string(),
            Value::Builtin("weakref_new".to_string()),
        );
        m
    }
    ...
    fn call_builtin(&mut self, name: &str, args: Vec<Value>, pos: &Position) -> Result<Value> {
        ...
        match name {
            ...
            "includes" => {
                ...
                match &args[0] {
                    Value::Array(arr) => {
                        Ok(Value::Bool(arr.borrow().iter().any(|v| v.equals(&args[1]))))
                    }
                    Value::Str(s) => {
            ...
            "filter" => {
                ...
                match &args[0] {
                    Value::Array(arr) => {
                        ...
                        for item in items {
                            if self
                                .call_value(callback.clone(), vec![item.clone()], None, pos)?
                                .is_truthy()
                            {
                                result.push(item);
                            }
            ...
            "split" => {
                ...
                match (&args[0], &args[1]) {
                    (Value::Str(s), Value::Str(sep)) => Ok(Value::make_array(
                        s.split(sep.as_str())
            ...
            "join" => {
                ...
                match (&args[0], &args[1]) {
                    (Value::Array(arr), Value::Str(sep)) => Ok(Value::Str(
                        arr.borrow()
                            .iter()
                            .map(|v| v.to_string())
                            .collect::<Vec<_>>()
                ...
                match (&args[0], &args[1]) {
                    (Value::Str(s), Value::Number(len)) => {
                        ...
                        if cur_len >= target {
                            ...
                            let pad: String =
                                std::iter::repeat(pad_char).take(target - cur_len).collect();
                            Ok(Value::Str(format!("{pad}{s}")))
                        }
                    }
                    _ => Err(CustomLangError::type_err(
                        "pad_start() requires (string, number[, string])",
                    )),
                }
            }
            "pad_end" => {
                ...
                match (&args[0], &args[1]) {
                    ...
                    _ => Err(CustomLangError::type_err(
                        "pad_end() requires (string, number[, string])",
                    )),
                }
            }
            "keys" => {
                ...
                match &args[0] {
                    Value::Object(obj) => Ok(Value::make_array(
                        obj.borrow().keys().map(|k| Value::Str(k.clone())).collect(),
                    )),
            ...
            "values" => {
                ...
                match &args[0] {
                    Value::Object(obj) => {
                        Ok(Value::make_array(obj.borrow().values().cloned().collect()))
                    }
                ...
                match &args[0] {
                    Value::Array(arr) => {
                        ...
                        for item in items {
                            ...
                            match mapped {
                                Value::Array(a) => result.extend(a.borrow().clone()),
                                v => result.push(v),
                            }
                        }
                        Ok(Value::make_array(result))
            ...
            "zip" => {
                ...
                let arrays: Vec<Vec<Value>> = args
                    ...
                    .map(|a| match a {
                        ...
                        _ => Err(CustomLangError::type_err("zip() requires arrays")),
                    })
                    .collect::<Result<_>>()?;
                let len = arrays.iter().map(|a| a.len()).min().unwrap_or(0);
                let result: Vec<Value> = (0..len)
                    .map(|i| Value::make_array(arrays.iter().map(|a| a[i].clone()).collect()))
                    .collect();
                Ok(Value::make_array(result))
            }
            "chunk" => {
                ...
                match (&args[0], &args[1]) {
                    (Value::Array(arr), Value::Number(n)) => {
                        let size = *n as usize;
                        if size == 0 {
                            return Err(CustomLangError::runtime("chunk() size must be > 0"));
                        }
                        let items = arr.borrow().clone();
                        let result: Vec<Value> = items
                            .chunks(size)
                            .map(|c| Value::make_array(c.to_vec()))
                            .collect();
                        Ok(Value::make_array(result))
                    }
                    _ => Err(CustomLangError::type_err(
                        "chunk() requires (array, number)",
                    )),
                }
            }
            "unique" => {
            ...
            "count" => {
                ...
                match &args[0] {
                    Value::Array(arr) => {
                        ...
                        for item in items {
                            if self
                                .call_value(cb.clone(), vec![item], None, pos)?
                                .is_truthy()
                            {
                                n += 1;
                            }
            ...
            "json_parse" => {
                ...
                match &args[0] {
                    Value::Str(s) => parse_json(s)
                        .map_err(|e| CustomLangError::runtime(format!("json_parse(): {e}"))),
            ...
            "json_is_valid" => {
                ...
                match &args[0] {
                    Value::Str(s) => Ok(Value::Bool(parse_json(s).is_ok())),
                    _ => Ok(Value::Bool(false)),
                }
            }
            ...
            "random_int" => {
                ...
                match (&args[0], &args[1]) {
                    ...
                    _ => Err(CustomLangError::type_err(
                        "random_int() requires (number, number)",
                    )),
                }
            }
            "random_bool" => {
            ...
            "math_clamp" => {
                ...
                match (&args[0], &args[1], &args[2]) {
                    (Value::Number(v), Value::Number(min), Value::Number(max)) => {
                        Ok(Value::Number(v.clamp(*min, *max)))
                    }
                    _ => Err(CustomLangError::type_err(
                        "clamp() requires (number, number, number)",
                    )),
                }
            }
            "math_sign" => {
                ...
                match &args[0] {
                    ...
                    _ => Err(CustomLangError::type_err("sign() requires number")),
                }
            }
            "math_hypot" => {
                if argc != 2 {
            ...
            "math_factorial" => {
                ...
                match &args[0] {
                    Value::Number(n) => {
                        if *n < 0.0 {
                            return Err(CustomLangError::runtime(
                                "factorial() requires non-negative number",
                            ));
                        }
                        let mut result = 1u64;
                        for i in 1..=(*n as u64) {
            ...
            "math_lerp" => {
                ...
                match (&args[0], &args[1], &args[2]) {
                    ...
                    _ => Err(CustomLangError::type_err("lerp() requires 3 numbers")),
                }
            }
            "math_degrees" => {
                if argc != 1 {
                ...
                match args[0] {
                    Value::Number(r) => Ok(Value::Number(r.to_degrees())),
                    _ => Err(CustomLangError::type_err("degrees() requires number")),
                }
            }
            "math_radians" => {
                if argc != 1 {
                ...
                match args[0] {
                    Value::Number(d) => Ok(Value::Number(d.to_radians())),
                    _ => Err(CustomLangError::type_err("radians() requires number")),
                }
            }
            "math_atan2" => {
                if argc != 2 {
                ...
                match (&args[0], &args[1]) {
                    ...
                    _ => Err(CustomLangError::type_err("atan2() requires 2 numbers")),
                }
            }
            "math_asin" => {
                if argc != 1 {
                ...
                match args[0] {
                    ...
                    _ => Err(CustomLangError::type_err("asin() requires number")),
                }
            }
            "math_acos" => {
                if argc != 1 {
                ...
                match args[0] {
                    ...
                    _ => Err(CustomLangError::type_err("acos() requires number")),
                }
            }
            "math_atan" => {
                if argc != 1 {
                ...
                match args[0] {
                    ...
                    _ => Err(CustomLangError::type_err("atan() requires number")),
                }
            }
            "math_exp" => {
                if argc != 1 {
                ...
                match args[0] {
                    ...
                    _ => Err(CustomLangError::type_err("exp() requires number")),
                }
            }
            "math_log2" => {
                if argc != 1 {
                ...
                match args[0] {
                    ...
                    _ => Err(CustomLangError::type_err("log2() requires number")),
                }
            }
            "math_log10" => {
                if argc != 1 {
                ...
                match args[0] {
                    ...
                    _ => Err(CustomLangError::type_err("log10() requires number")),
                }
            }
            "math_cbrt" => {
                if argc != 1 {
                ...
                match args[0] {
                    ...
                    _ => Err(CustomLangError::type_err("cbrt() requires number")),
                }
            }
            "math_trunc" => {
                if argc != 1 {
                ...
                match args[0] {
                    ...
                    _ => Err(CustomLangError::type_err("trunc() requires number")),
                }
            }
            ...
            "str_at" => {
                ...
                match (&args[0], &args[1]) {
                    ...
                    _ => Err(CustomLangError::type_err("at() requires (string, number)")),
                }
            }
            "str_last_index_of" => {
                if argc != 2 {
                ...
                match (&args[0], &args[1]) {
                    (Value::Str(s), Value::Str(sub)) => Ok(Value::Number(
                        s.rfind(sub.as_str()).map(|i| i as f64).unwrap_or(-1.0),
                    )),
                    _ => Err(CustomLangError::type_err(
                        "last_index_of() requires (string, string)",
                    )),
                }
            }
            "str_char_codes" => {
                ...
                match &args[0] {
                    Value::Str(s) => Ok(Value::make_array(
                        s.chars().map(|c| Value::Number(c as u32 as f64)).collect(),
                    )),
                    _ => Err(CustomLangError::type_err("char_codes() requires string")),
                }
            }
            "str_from_char_codes" => {
                if argc != 1 {
                ...
                match &args[0] {
                    ...
                    _ => Err(CustomLangError::type_err(
                        "from_char_codes() requires array",
                    )),
                }
            }
            "str_is_digit" => {
                ...
                match &args[0] {
                    ...
                    _ => Err(CustomLangError::type_err("is_digit() requires string")),
                }
            }
            "str_is_alpha" => {
                if argc != 1 {
                ...
                match &args[0] {
                    ...
                    _ => Err(CustomLangError::type_err("is_alpha() requires string")),
                }
            }
            "str_is_alnum" => {
                if argc != 1 {
                ...
                match &args[0] {
                    ...
                    _ => Err(CustomLangError::type_err(
                        "is_alphanumeric() requires string",
                    )),
                }
            }
            "str_is_whitespace" => {
                ...
                match &args[0] {
                    ...
                    _ => Err(CustomLangError::type_err("is_whitespace() requires string")),
                }
            }
            "str_is_upper" => {
                if argc != 1 {
                ...
                match &args[0] {
                    Value::Str(s) => Ok(Value::Bool(s.chars().all(|c| c.is_uppercase()))),
                    _ => Err(CustomLangError::type_err("is_upper() requires string")),
                }
            }
            "str_is_lower" => {
                if argc != 1 {
                ...
                match &args[0] {
                    Value::Str(s) => Ok(Value::Bool(s.chars().all(|c| c.is_lowercase()))),
                    _ => Err(CustomLangError::type_err("is_lower() requires string")),
                }
            }
            "str_count_occurrences" => {
                if argc != 2 {
                ...
                match (&args[0], &args[1]) {
                    ...
                    _ => Err(CustomLangError::type_err(
                        "count_occurrences() requires (string, string)",
                    )),
                }
            }
            "str_reverse" => {
                ...
                match &args[0] {
                    ...
                    _ => Err(CustomLangError::type_err(
                        "reverse_string() requires string",
                    )),
                }
            }
            "str_word_count" => {
                ...
                match &args[0] {
                    Value::Str(s) => Ok(Value::Number(s.split_whitespace().count() as f64)),
                    _ => Err(CustomLangError::type_err("word_count() requires string")),
                }
            }
            "str_lines" => {
                if argc != 1 {
                ...
                match &args[0] {
                    ...
                    _ => Err(CustomLangError::type_err("lines() requires string")),
                }
            }
            ...
            "arr_unzip" => {
                ...
                match &args[0] {
                    Value::Array(arr) => {
                        ...
                        Ok(Value::make_array(vec![
                            Value::make_array(a),
                            Value::make_array(b),
                        ]))
                    }
                    _ => Err(CustomLangError::type_err("unzip() requires array")),
                }
            }
            "arr_group_by" => {
                if argc != 2 {
                ...
                match &args[0] {
                    Value::Array(arr) => {
                        ...
                        for item in items {
                            let k = self.call_value(cb.clone(), vec![item.clone()], None, pos)?;
                            groups.entry(k.to_string()).or_default().push(item);
                        }
                        let mut map = HashMap::new();
                        for (k, v) in groups {
                            map.insert(k, Value::make_array(v));
                        }
                        Ok(Value::make_object(map))
                    }
                    _ => Err(CustomLangError::type_err("group_by() requires array")),
                }
            }
            "arr_partition" => {
                if argc != 2 {
                ...
                match &args[0] {
                    Value::Array(arr) => {
                        ...
                        for item in items {
                            if self
                                .call_value(cb.clone(), vec![item.clone()], None, pos)?
                                .is_truthy()
                            {
                                yes.push(item);
                            } else {
                        ...
                        Ok(Value::make_array(vec![
                            Value::make_array(yes),
                            Value::make_array(no),
                        ]))
                    }
                    _ => Err(CustomLangError::type_err("partition() requires array")),
                }
            }
            "arr_rotate" => {
                if argc != 2 {
            ...
            "arr_take" => {
                ...
                match (&args[0], &args[1]) {
                    ...
                    _ => Err(CustomLangError::type_err("take() requires (array, number)")),
                }
            }
            "arr_drop" => {
                if argc != 2 {
                ...
                match (&args[0], &args[1]) {
                    ...
                    _ => Err(CustomLangError::type_err("drop() requires (array, number)")),
                }
            }
            "arr_take_while" => {
                if argc != 2 {
                ...
                match &args[0] {
                    ...
                    _ => Err(CustomLangError::type_err("take_while() requires array")),
                }
            }
            "arr_drop_while" => {
                if argc != 2 {
                ...
                match &args[0] {
                    ...
                    _ => Err(CustomLangError::type_err("drop_while() requires array")),
                }
            }
            "arr_flatten_deep" => {
                if argc != 1 {
                ...
                fn fd(v: &Value) -> Vec<Value> {
                    match v {
                        Value::Array(a) => a.borrow().iter().flat_map(fd).collect(),
                        _ => vec![v.clone()],
                    }
                }
                match &args[0] {
                    Value::Array(_) => Ok(Value::make_array(fd(&args[0]))),
                    _ => Err(CustomLangError::type_err("flatten_deep() requires array")),
                }
            }
            "arr_fill" => {
                if argc != 2 {
                ...
                match (&args[0], &args[1]) {
                    (Value::Number(n), v) => Ok(Value::make_array(vec![v.clone(); *n as usize])),
                    _ => Err(CustomLangError::type_err("fill() requires (number, value)")),
                }
            }
            "arr_fill_with" => {
                if argc != 2 {
            ...
            "arr_min_by" => {
                ...
                match &args[0] {
                    Value::Array(arr) => {
                        ...
                        let mut min_val =
                            self.call_value(cb.clone(), vec![min_item.clone()], None, pos)?;
                        for item in items[1..].iter() {
                            let v = self.call_value(cb.clone(), vec![item.clone()], None, pos)?;
                            if let (Value::Number(a), Value::Number(b)) = (&v, &min_val) {
                                if a < b {
                                    min_val = v;
                                    min_item = item.clone();
                                }
                    ...
                    _ => Err(CustomLangError::type_err("min_by() requires array")),
                }
            }
            "arr_max_by" => {
                if argc != 2 {
                ...
                match &args[0] {
                    Value::Array(arr) => {
                        ...
                        let mut max_val =
                            self.call_value(cb.clone(), vec![max_item.clone()], None, pos)?;
                        for item in items[1..].iter() {
                            let v = self.call_value(cb.clone(), vec![item.clone()], None, pos)?;
                            if let (Value::Number(a), Value::Number(b)) = (&v, &max_val) {
                                if a > b {
                                    max_val = v;
                                    max_item = item.clone();
                                }
                    ...
                    _ => Err(CustomLangError::type_err("max_by() requires array")),
                }
            }
            "arr_sort_by" => {
                if argc != 2 {
                ...
                match &args[0] {
                    Value::Array(arr) => {
                        ...
                        items.sort_by(|a, b| {
                            ...
                            let av = self.call_value(cb.clone(), vec![a.clone()], None, pos);
                            let bv = self.call_value(cb.clone(), vec![b.clone()], None, pos);
                            match (av, bv) {
                                (Ok(Value::Number(an)), Ok(Value::Number(bn))) => {
                                    an.partial_cmp(&bn).unwrap_or(std::cmp::Ordering::Equal)
                                }
                                (Ok(Value::Str(as_)), Ok(Value::Str(bs))) => as_.cmp(&bs),
                                (Err(e), _) | (_, Err(e)) => {
                                    error = Some(e);
                                    std::cmp::Ordering::Equal
                                }
                    ...
                    _ => Err(CustomLangError::type_err("sort_by() requires array")),
                }
            }
            "arr_difference" => {
                if argc != 2 {
                ...
                match (&args[0], &args[1]) {
                    (Value::Array(a), Value::Array(b)) => {
                        let bi = b.borrow().clone();
                        let result: Vec<Value> = a
                            .borrow()
                            .iter()
                            .filter(|v| !bi.iter().any(|bv| v.equals(bv)))
                            .cloned()
                            .collect();
                        Ok(Value::make_array(result))
                    }
                    _ => Err(CustomLangError::type_err("difference() requires 2 arrays")),
                }
            }
            "arr_intersection" => {
                if argc != 2 {
                ...
                match (&args[0], &args[1]) {
                    ...
                    _ => Err(CustomLangError::type_err(
                        "intersection() requires 2 arrays",
                    )),
                }
            }
            "arr_union" => {
                ...
                match (&args[0], &args[1]) {
                    ...
                    _ => Err(CustomLangError::type_err("union() requires 2 arrays")),
                }
            }
            ...
            "obj_get_path" => {
                ...
                match (&args[0], &args[1]) {
                    ...
                    _ => Err(CustomLangError::type_err(
                        "get_path() requires (object, string)",
                    )),
                }
            }
            "obj_set_path" => {
                ...
                match (&args[0], &args[1]) {
                    (Value::Object(obj), Value::Str(path)) => {
                        let val = args[2].clone();
                        let keys: Vec<&str> = path.split('.').collect();
                        let mut current = obj.clone();
                        for (i, key) in keys.iter().enumerate() {
                            if i == keys.len() - 1 {
                                current.borrow_mut().insert(key.to_string(), val.clone());
                            } else {
                                let next = current
                                    .borrow()
                                    .get(*key)
                                    .cloned()
                                    .unwrap_or_else(|| Value::make_object(HashMap::new()));
                    ...
                    _ => Err(CustomLangError::type_err(
                        "set_path() requires (object, string, value)",
                    )),
                }
            }
            "obj_omit" => {
                ...
                match (&args[0], &args[1]) {
                    ...
                    _ => Err(CustomLangError::type_err("omit() requires (object, array)")),
                }
            }
            "obj_pick" => {
                if argc != 2 {
                ...
                match (&args[0], &args[1]) {
                    ...
                    _ => Err(CustomLangError::type_err("pick() requires (object, array)")),
                }
            }
            "obj_map_values" => {
                if argc != 2 {
                ...
                match &args[0] {
                    Value::Object(obj) => {
                        ...
                        for (k, v) in obj.borrow().iter() {
                            map.insert(
                                k.clone(),
                                self.call_value(cb.clone(), vec![v.clone()], None, pos)?,
                            );
                        }
                        Ok(Value::make_object(map))
                    }
                    _ => Err(CustomLangError::type_err("map_values() requires object")),
                }
            }
            "obj_map_keys" => {
                if argc != 2 {
                ...
                match &args[0] {
                    Value::Object(obj) => {
                        ...
                        for (k, v) in obj.borrow().iter() {
                            let new_k = self
                                .call_value(cb.clone(), vec![Value::Str(k.clone())], None, pos)?
                                .to_string();
                            map.insert(new_k, v.clone());
                        }
                        Ok(Value::make_object(map))
                    }
                    _ => Err(CustomLangError::type_err("map_keys() requires object")),
                }
            }
            "obj_filter_values" => {
                if argc != 2 {
                ...
                match &args[0] {
                    Value::Object(obj) => {
                        ...
                        for (k, v) in obj.borrow().iter() {
                            if self
                                .call_value(cb.clone(), vec![v.clone()], None, pos)?
                                .is_truthy()
                            {
                                map.insert(k.clone(), v.clone());
                            }
                    ...
                    _ => Err(CustomLangError::type_err("filter_values() requires object")),
                }
            }
            "obj_invert" => {
                if argc != 1 {
                ...
                match &args[0] {
                    Value::Object(obj) => {
                        let map: HashMap<String, Value> = obj
                            .borrow()
                            .iter()
                            .map(|(k, v)| (v.to_string(), Value::Str(k.clone())))
                            .collect();
                        Ok(Value::make_object(map))
                    }
                    _ => Err(CustomLangError::type_err("invert() requires object")),
                }
            }
            "obj_from_entries" => {
                if argc != 1 {
                ...
                match &args[0] {
                    ...
                    _ => Err(CustomLangError::type_err("from_entries() requires array")),
                }
            }
            ...
            "fs_read_text" => {
                ...
                match &args[0] {
                    Value::Str(p) => std::fs::read_to_string(p)
                        .map(Value::Str)
                        .map_err(|e| CustomLangError::io_err(format!("read: {e}"))),
                    _ => Err(CustomLangError::type_err("read_text() requires string")),
                }
            }
            "fs_write_text" => {
                if argc != 2 {
                ...
                match (&args[0], &args[1]) {
                    (Value::Str(p), c) => {
                        std::fs::write(p, c.to_string())
                            .map_err(|e| CustomLangError::io_err(format!("write: {e}")))?;
                        Ok(Value::Bool(true))
                    }
                    _ => Err(CustomLangError::type_err(
                ...
                match (&args[0], &args[1]) {
                    (Value::Str(p), c) => {
                        ...
                        let mut f = std::fs::OpenOptions::new()
                            ...
                            .append(true)
                            .open(p)
                            .map_err(|e| CustomLangError::io_err(format!("append: {e}")))?;
                        write!(f, "{}", c)
                            .map_err(|e| CustomLangError::io_err(format!("append: {e}")))?;
                        Ok(Value::Bool(true))
                    }
                    _ => Err(CustomLangError::type_err(
                        "append_text() requires (string, value)",
                    )),
                }
            }
            "fs_exists" => {
                ...
                match &args[0] {
                    ...
                    _ => Err(CustomLangError::type_err("exists() requires string")),
                }
            }
            "fs_delete" => {
                if argc != 1 {
                ...
                match &args[0] {
                    Value::Str(p) => {
                        ...
                        .map_err(|e| CustomLangError::io_err(format!("delete: {e}")))?;
                        Ok(Value::Bool(true))
                    }
                    _ => Err(CustomLangError::type_err("delete() requires string")),
                }
            }
            "fs_rename" => {
                if argc != 2 {
                ...
                match (&args[0], &args[1]) {
                    (Value::Str(from), Value::Str(to)) => {
                        std::fs::rename(from, to)
                            .map_err(|e| CustomLangError::io_err(format!("rename: {e}")))?;
                        Ok(Value::Bool(true))
                    }
                    _ => Err(CustomLangError::type_err(
                        "rename() requires (string, string)",
                    )),
                }
            }
            "fs_copy" => {
                ...
                match (&args[0], &args[1]) {
                    (Value::Str(from), Value::Str(to)) => {
                        std::fs::copy(from, to)
                            .map_err(|e| CustomLangError::io_err(format!("copy: {e}")))?;
                        Ok(Value::Bool(true))
                    }
                    _ => Err(CustomLangError::type_err(
                        "copy() requires (string, string)",
                    )),
                }
            }
            "fs_mkdir" => {
                ...
                match &args[0] {
                    Value::Str(p) => {
                        std::fs::create_dir(p)
                            .map_err(|e| CustomLangError::io_err(format!("mkdir: {e}")))?;
                        Ok(Value::Bool(true))
                    }
                    _ => Err(CustomLangError::type_err("mkdir() requires string")),
                }
            }
            "fs_mkdir_all" => {
                if argc != 1 {
                ...
                match &args[0] {
                    Value::Str(p) => {
                        std::fs::create_dir_all(p)
                            .map_err(|e| CustomLangError::io_err(format!("mkdir_all: {e}")))?;
                        Ok(Value::Bool(true))
                    }
                    _ => Err(CustomLangError::type_err("mkdir_all() requires string")),
                }
            }
            "fs_rmdir" => {
                if argc != 1 {
                ...
                match &args[0] {
                    Value::Str(p) => {
                        std::fs::remove_dir(p)
                            .map_err(|e| CustomLangError::io_err(format!("rmdir: {e}")))?;
                        Ok(Value::Bool(true))
                    }
                    _ => Err(CustomLangError::type_err("rmdir() requires string")),
                }
            }
            "fs_list_dir" => {
                if argc != 1 {
                ...
                match &args[0] {
                    Value::Str(p) => {
                        let entries: Vec<Value> = std::fs::read_dir(p)
                            .map_err(|e| CustomLangError::io_err(format!("list_dir: {e}")))?
                            .filter_map(|e| e.ok())
                            .map(|e| Value::Str(e.file_name().to_string_lossy().to_string()))
                            .collect();
                        Ok(Value::make_array(entries))
                    }
                    _ => Err(CustomLangError::type_err("list_dir() requires string")),
                }
            }
            "fs_is_file" => {
                if argc != 1 {
                ...
                match &args[0] {
                    Value::Str(p) => Ok(Value::Bool(std::path::Path::new(p).is_file())),
                    _ => Err(CustomLangError::type_err("is_file() requires string")),
                }
            }
            "fs_is_dir" => {
                if argc != 1 {
                ...
                match &args[0] {
                    Value::Str(p) => Ok(Value::Bool(std::path::Path::new(p).is_dir())),
                    _ => Err(CustomLangError::type_err("is_dir() requires string")),
                }
            }
            "fs_file_size" => {
                if argc != 1 {
                ...
                match &args[0] {
                    Value::Str(p) => {
                        let meta = std::fs::metadata(p)
                            .map_err(|e| CustomLangError::io_err(format!("file_size: {e}")))?;
                        Ok(Value::Number(meta.len() as f64))
                    }
                    _ => Err(CustomLangError::type_err("file_size() requires string")),
                }
            }
            "fs_last_modified" => {
                if argc != 1 {
                ...
                match &args[0] {
                    Value::Str(p) => {
                        let meta = std::fs::metadata(p)
                            .map_err(|e| CustomLangError::io_err(format!("last_modified: {e}")))?;
                        let t = meta
                            .modified()
                            .map_err(|e| CustomLangError::io_err(format!("last_modified: {e}")))?;
                    ...
                    _ => Err(CustomLangError::type_err("last_modified() requires string")),
                }
            }
            "fs_temp_file" => {
                let t = std::env::temp_dir().join(format!("cl_tmp_{}", lcg_rand(42) % 1000000));
                Ok(Value::Str(t.to_string_lossy().to_string()))
            }
            "fs_temp_dir" => Ok(Value::Str(
            ...
            "path_join" => {
                let parts: Vec<String> = args.iter().map(|v| v.to_string()).collect();
                let p = parts.iter().fold(std::path::PathBuf::new(), |mut acc, p| {
                    acc.push(p);
                    acc
                });
            ...
            "path_dirname" => {
                ...
                match &args[0] {
                    Value::Str(p) => Ok(Value::Str(
                        std::path::Path::new(p)
                            .parent()
                            .map(|p| p.to_string_lossy().to_string())
                            .unwrap_or(".".to_string()),
                    )),
                    _ => Err(CustomLangError::type_err("dirname() requires string")),
                }
            }
            "path_basename" => {
                if argc != 1 {
                ...
                match &args[0] {
                    ...
                    _ => Err(CustomLangError::type_err("basename() requires string")),
                }
            }
            "path_stem" => {
                if argc != 1 {
                ...
                match &args[0] {
                    ...
                    _ => Err(CustomLangError::type_err("stem() requires string")),
                }
            }
            "path_extension" => {
                if argc != 1 {
                ...
                match &args[0] {
                    ...
                    _ => Err(CustomLangError::type_err("extension() requires string")),
                }
            }
            "path_absolute" => {
                if argc != 1 {
                ...
                match &args[0] {
                    ...
                    _ => Err(CustomLangError::type_err("absolute() requires string")),
                }
            }
            "path_normalize" => {
                if argc != 1 {
                ...
                match &args[0] {
                    Value::Str(p) => {
                        ...
                        Ok(Value::Str(result.to_string_lossy().to_string()))
                    }
                    _ => Err(CustomLangError::type_err("normalize() requires string")),
                }
            }
            "path_split" => {
                if argc != 1 {
                ...
                match &args[0] {
                    Value::Str(p) => Ok(Value::make_array(
                        std::path::Path::new(p)
                            .components()
                            .map(|c| Value::Str(c.as_os_str().to_string_lossy().to_string()))
                            .collect(),
                    )),
                    _ => Err(CustomLangError::type_err("split() requires string")),
                }
            }
            "path_is_absolute" => {
                if argc != 1 {
                ...
                match &args[0] {
                    Value::Str(p) => Ok(Value::Bool(std::path::Path::new(p).is_absolute())),
                    _ => Err(CustomLangError::type_err("is_absolute() requires string")),
                }
            }

            // â”€â”€ Process â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            "proc_args" => Ok(Value::make_array(
            ...
            "proc_env" => {
                ...
                match &args[0] {
                    Value::Str(k) => Ok(std::env::var(k).map(Value::Str).unwrap_or(Value::Null)),
                    _ => Err(CustomLangError::type_err("env() requires string")),
                }
            }
            "proc_env_all" => {
                let mut map = HashMap::new();
                ...
                match &args[0] {
                    Value::Str(p) => {
                        std::env::set_current_dir(p)
                            .map_err(|e| CustomLangError::io_err(format!("chdir: {e}")))?;
                        Ok(Value::Bool(true))
                    }
                    _ => Err(CustomLangError::type_err("chdir() requires string")),
                }
            }
            "proc_pid" => Ok(Value::Number(std::process::id() as f64)),
            "proc_platform" => Ok(Value::Str(
                if cfg!(windows) {
                    "windows"
            ...
            "proc_run" => {
                ...
                match &args[0] {
                    Value::Str(cmd) => {
                        ...
                        map.insert(
                            "exit_code".to_string(),
                            Value::Number(output.status.code().unwrap_or(0) as f64),
                        );
                        Ok(Value::make_object(map))
                    }
            ...
            "http_get" => {
                ...
                match &args[0] {
                    Value::Str(url) => match ureq::get(url).call() {
                        Ok(resp) => {
                ...
                match (&args[0], &args[1]) {
                    (Value::Str(url), body) => {
                        ...
                        match ureq::post(url).send_string(&body_str) {
                            ...
                            Err(e) => {
                                Err(CustomLangError::runtime(format!("http.post failed: {e}")))
                            }
                        }
                    }
                    _ => Err(CustomLangError::type_err(
                        "http.post() requires (string, value)",
                    )),
                }
            }
                ...
                match &args[0] {
                    Value::Str(url) => {
                        ...
                        match ureq::request(&method, url).call() {
                            ...
                            Err(e) => Err(CustomLangError::runtime(format!(
                                "http.{method} failed: {e}"
                            ))),
                        }
                    }
            ...
            "enc_url_encode" => {
                ...
                match &args[0] {
                    Value::Str(s) => Ok(Value::Str(url_encode(s))),
                    _ => Err(CustomLangError::type_err("url_encode() requires string")),
                }
            }
            "enc_url_decode" => {
                if argc != 1 {
                ...
                match &args[0] {
                    Value::Str(s) => Ok(Value::Str(url_decode(s))),
                    _ => Err(CustomLangError::type_err("url_decode() requires string")),
                }
            }
            "enc_html_encode" => {
                if argc != 1 {
                ...
                match &args[0] {
                    ...
                    _ => Err(CustomLangError::type_err("html_encode() requires string")),
                }
            }
            "enc_html_decode" => {
                if argc != 1 {
                ...
                match &args[0] {
                    ...
                    _ => Err(CustomLangError::type_err("html_decode() requires string")),
                }
            }
            ...
            "crypto_sha512" => {
                ...
                let s = args[0].to_string();
                Ok(Value::Str(sha512_hex(s.as_bytes())))
            }
            "crypto_md5" => {
                if argc != 1 {
                    return err_argc("1");
            ...
            "crypto_hmac_sha256" => {
                ...
                match (&args[0], &args[1]) {
                    (Value::Str(key), Value::Str(msg)) => {
                        Ok(Value::Str(hmac_sha256_hex(key.as_bytes(), msg.as_bytes())))
                    }
                    _ => Err(CustomLangError::type_err(
                        "hmac_sha256() requires (string, string)",
                    )),
                }
            }
            "crypto_base64_encode" => {
                ...
                Ok(Value::Str(base64_encode(args[0].to_string().as_bytes())))
            }
            "crypto_base64_decode" => {
                if argc != 1 {
                    return err_argc("1");
            ...
            "crypto_hex_encode" => {
                ...
                Ok(Value::Str(hex_encode(args[0].to_string().as_bytes())))
            }
            "crypto_hex_decode" => {
                if argc != 1 {
                    return err_argc("1");
            ...
            "crypto_random_bytes" => {
                ...
                match args[0] {
                    Value::Number(n) => {
                        let bytes: Vec<u8> = (0..n as usize).map(|_| lcg_rand(42) as u8).collect();
                        Ok(Value::Str(hex_encode(&bytes)))
                    }
                    _ => Err(CustomLangError::type_err("random_bytes() requires number")),
                }
            }
            "crypto_compare_secure" => {
                if argc != 2 {
                ...
                match (&args[0], &args[1]) {
                    ...
                    _ => Err(CustomLangError::type_err(
                        "compare_secure() requires (string, string)",
                    )),
                }
            }
            ...
            "regex_new" => {
                ...
                match &args[0] {
                    Value::Str(pattern) => {
                        // Store pattern as object
                        let mut map = HashMap::new();
                        map.insert("pattern".to_string(), Value::Str(pattern.clone()));
                        map.insert(
                            "test".to_string(),
                            Value::Builtin("regex_test_method".to_string()),
                        );
                        Ok(Value::make_object(map))
                    }
                    _ => Err(CustomLangError::type_err(
                        "regex.new() requires string pattern",
                    )),
                }
            }
            "regex_test" | "regex_test_method" => {
                ...
                let text = if let Value::Str(s) = &args[1] {
                    s.clone()
                } else {
                    args[1].to_string()
                };
                match pattern {
                    Some(p) => {
                        let re = regex::Regex::new(&p)
                            .map_err(|e| CustomLangError::runtime(format!("regex error: {e}")))?;
                        Ok(Value::Bool(re.is_match(&text)))
                    }
                    None => Err(CustomLangError::type_err(
                        "regex.test() requires pattern as first arg",
                    )),
                }
            }
            "regex_match" => {
                ...
                match pattern {
                    ...
                    None => Err(CustomLangError::type_err(
                        "regex.match() requires pattern as first arg",
                    )),
                }
            }
            "regex_match_all" => {
                ...
                match pattern {
                    ...
                    None => Err(CustomLangError::type_err(
                        "regex.match_all() requires pattern as first arg",
                    )),
                }
            }
            "regex_replace" => {
                ...
                let repl = if let Value::Str(s) = &args[2] {
                    s.clone()
                } else {
                    args[2].to_string()
                };
                match pattern {
                    Some(p) => {
                        let re = regex::Regex::new(&p)
                            .map_err(|e| CustomLangError::runtime(format!("regex error: {e}")))?;
                        Ok(Value::Str(re.replace(&text, repl.as_str()).to_string()))
                    }
                    None => Err(CustomLangError::type_err(
                        "regex.replace() requires pattern as first arg",
                    )),
                }
            }
            "regex_replace_all" => {
                ...
                match pattern {
                    ...
                    None => Err(CustomLangError::type_err(
                        "regex.replace_all() requires pattern as first arg",
                    )),
                }
            }
            ...
            "dt_now" => {
                ...
                map.insert(
                    ...
                    Value::Number(now.weekday().num_days_from_sunday() as f64),
                );
                Ok(Value::make_object(map))
            }
            "dt_from_timestamp" => {
                ...
                match args[0] {
                    ...
                    _ => Err(CustomLangError::type_err(
                        "from_timestamp() requires number",
                    )),
                }
            }
            "dt_new" => {
                ...
                match (&args[0], &args[1], &args[2]) {
                    (Value::Number(y), Value::Number(mo), Value::Number(d)) => {
                        ...
                        let dt = chrono::Local
                            .with_ymd_and_hms(*y as i32, *mo as u32, *d as u32, h, mi, s)
                            .earliest()
                            .unwrap_or_default();
                        let mut map = HashMap::new();
                        ...
                        map.insert(
                            "timestamp".to_string(),
                            Value::Number(dt.timestamp_millis() as f64),
                        );
                        Ok(Value::make_object(map))
                    }
            ...
            "dt_format" | "dt_parse" => Ok(Value::Str(format!(
                "{}",
            ...
            "coll_queue" => {
                ...
                map.insert(
                    "enqueue".to_string(),
                    Value::Builtin("coll_queue_enqueue".to_string()),
                );
                map.insert(
                    "dequeue".to_string(),
                    Value::Builtin("coll_queue_dequeue".to_string()),
                );
                map.insert(
                    "peek".to_string(),
                    Value::Builtin("coll_queue_peek".to_string()),
                );
                map.insert(
                    "is_empty".to_string(),
                    Value::Builtin("coll_is_empty".to_string()),
                );
            ...
            "coll_stack" => {
                ...
                map.insert(
                    "push".to_string(),
                    Value::Builtin("coll_stack_push".to_string()),
                );
                map.insert(
                    "pop".to_string(),
                    Value::Builtin("coll_stack_pop".to_string()),
                );
                map.insert(
                    "peek".to_string(),
                    Value::Builtin("coll_stack_peek".to_string()),
                );
                map.insert(
                    "is_empty".to_string(),
                    Value::Builtin("coll_is_empty".to_string()),
            ...
            "coll_deque" => {
                ...
                map.insert(
                    ...
                    Value::Builtin("coll_deque_push_front".to_string()),
                );
                map.insert(
                    "push_back".to_string(),
                    Value::Builtin("coll_deque_push_back".to_string()),
                );
                map.insert(
                    "pop_front".to_string(),
                    Value::Builtin("coll_deque_pop_front".to_string()),
                );
                map.insert(
                    "pop_back".to_string(),
                    Value::Builtin("coll_deque_pop_back".to_string()),
                );
                map.insert(
                    "is_empty".to_string(),
                    Value::Builtin("coll_is_empty".to_string()),
            ...
            "coll_linked_list" => {
                ...
                map.insert(
                    "to_array".to_string(),
                    Value::Builtin("coll_to_array".to_string()),
                );
                Ok(Value::make_object(map))
            }
            "coll_is_empty" => {
                ...
                match &args[0] {
                    Value::Object(o) => {
                        let d = o.borrow();
                        let data = d.get("_data");
                        Ok(Value::Bool(match data {
                            Some(Value::Array(a)) => a.borrow().is_empty(),
                            _ => true,
                        }))
                    }
                    _ => Err(CustomLangError::type_err("is_empty requires collection")),
                }
            }
            "coll_to_array" => {
                if argc != 1 {
                ...
                match &args[0] {
                    Value::Object(o) => {
                        let d = o.borrow();
                        Ok(d.get("_data").cloned().unwrap_or(Value::make_array(vec![])))
                    }
                    _ => Err(CustomLangError::type_err("to_array requires collection")),
                }
            }
            "coll_queue_enqueue" | "coll_stack_push" | "coll_deque_push_back" => {
                ...
                match &args[0] {
                    Value::Object(o) => {
                        if let Some(Value::Array(a)) = o.borrow().get("_data") {
                            a.borrow_mut().push(args[1].clone());
                        }
                        Ok(Value::Null)
                    }
                    _ => Err(CustomLangError::type_err("push requires collection")),
                }
            }
            "coll_deque_push_front" => {
                ...
                match &args[0] {
                    ...
                    _ => Err(CustomLangError::type_err("push_front requires collection")),
                }
            }
            "coll_queue_dequeue" | "coll_deque_pop_front" => {
                ...
                match &args[0] {
                    Value::Object(o) => {
                        if let Some(Value::Array(a)) = o.borrow().get("_data") {
                            let r = if a.borrow().is_empty() {
                                Value::Null
                            } else {
                                a.borrow_mut().remove(0)
                            };
                            Ok(r)
                        } else {
                            Ok(Value::Null)
                    ...
                    _ => Err(CustomLangError::type_err("dequeue requires collection")),
                }
            }
            "coll_stack_pop" | "coll_deque_pop_back" => {
                ...
                match &args[0] {
                    ...
                    _ => Err(CustomLangError::type_err("pop requires collection")),
                }
            }
            "coll_queue_peek" | "coll_stack_peek" => {
                ...
                match &args[0] {
                    Value::Object(o) => {
                        if let Some(Value::Array(a)) = o.borrow().get("_data") {
                            Ok(a.borrow().last().cloned().unwrap_or(Value::Null))
                        } else {
                            Ok(Value::Null)
                        }
                    }
                    _ => Err(CustomLangError::type_err("peek requires collection")),
                }
            }
            ...
            "test_describe" => {
                ...
                match &args[0] {
                    ...
                    _ => Err(CustomLangError::type_err(
                        "describe() requires (string, function)",
                    )),
                }
            }
            "test_assert_eq" => {
            ...
            "test_assert_throws" => {
                ...
                match self.call_value(cb, vec![], None, pos) {
                    Err(_) => Ok(Value::Null),
                    Ok(_) => Err(CustomLangError::runtime(
                        "assert_throws: expected exception but none was thrown",
                    )),
                }
            }
            "test_assert_true" => {
            ...
            "update" => {
                ...
                match (&args[0], &args[1]) {
                    (Value::Object(orig), Value::Object(updates)) => {
                        ...
                        for (k, v) in updates.borrow().iter() {
                            map.insert(k.clone(), v.clone());
                        }
                        Ok(Value::make_object(map))
                    }
                    _ => Err(CustomLangError::type_err(
                        "update() requires (object, object)",
                    )),
                }
            }
            "set_at" => {
                ...
                match (&args[0], &args[1]) {
                    ...
                    _ => Err(CustomLangError::type_err(
                        "set_at() requires (array, number, value)",
                    )),
                }
            }
            ...
            "Ok" => {
                ...
                let mut m = HashMap::new();
                m.insert("__ok__".to_string(), Value::Bool(true));
                m.insert("value".to_string(), args[0].clone());
                Ok(Value::make_object(m))
            }
            "Err" => {
                if argc != 1 {
                    return err_argc("1");
                ...
                let mut m = HashMap::new();
                m.insert("__ok__".to_string(), Value::Bool(false));
                m.insert("error".to_string(), args[0].clone());
                Ok(Value::make_object(m))
            }
            "Some" => {
                if argc != 1 {
                ...
                let mut m = HashMap::new();
                m.insert("__some__".to_string(), Value::Bool(true));
                m.insert("value".to_string(), args[0].clone());
                Ok(Value::make_object(m))
            }
            "None_val" => {
                let mut m = HashMap::new();
                m.insert("__some__".to_string(), Value::Bool(false));
                Ok(Value::make_object(m))
            }
            "is_ok" => {
                if argc != 1 {
                ...
                match &args[0] {
                    Value::Object(o) => Ok(Value::Bool(matches!(
                        o.borrow().get("__ok__"),
            ...
            "unwrap" => {
                ...
                match &args[0] {
                    Value::Object(o) => {
                        let b = o.borrow();
                        if matches!(b.get("__ok__"), Some(Value::Bool(true))) {
                            Ok(b.get("value").cloned().unwrap_or(Value::Null))
                        } else if let Some(v) = b.get("value").or(b.get("error")) {
                            Err(CustomLangError::runtime(format!("unwrap on Err: {}", v)))
                        } else {
                            Err(CustomLangError::runtime("unwrap failed"))
                        }
                    }
                    _ => Ok(args[0].clone()),
                }
            ...
            "__promise_resolve__" => {
                if argc == 1 {
                    Env::define(&self.env, "__promise_resolved__", args[0].clone());
                }
                Ok(Value::Null)
            }
            "__promise_reject__" => {
                if argc == 1 {
                    Env::define(&self.env, "__promise_rejected__", args[0].clone());
                }
                Ok(Value::Null)
            }
            ...
            "fmt" => {
                ...
                match &args[0] {
                    ...
                    _ => Err(CustomLangError::type_err(
                        "fmt() first argument must be string",
                    )),
                }
            }
            ...
            "Map" | "map_new" => {
                ...
                map.insert(
                    "delete".to_string(),
                    Value::Builtin("map_delete".to_string()),
                );
                map.insert("keys".to_string(), Value::Builtin("map_keys".to_string()));
                map.insert(
                    "values".to_string(),
                    Value::Builtin("map_values".to_string()),
                );
                map.insert(
                    "entries".to_string(),
                    Value::Builtin("map_entries".to_string()),
                );
            ...
            "map_has" => {
                ...
                match &args[0] {
                    Value::Object(o) => {
                        let data = o.borrow().get("_data").cloned();
                        if let Some(Value::Object(d)) = data {
                            Ok(Value::Bool(d.borrow().contains_key(&args[1].to_string())))
                        } else {
                            Ok(Value::Bool(false))
                        }
                    }
            ...
            "map_vals" | "map_values" => {
                ...
                match &args[0] {
                    Value::Object(o) => {
                        let data = o.borrow().get("_data").cloned();
                        if let Some(Value::Object(d)) = data {
                            Ok(Value::make_array(d.borrow().values().cloned().collect()))
                        } else {
                            Ok(Value::make_array(vec![]))
                        }
                    }
                        ...
                        if let Some(Value::Object(d)) = data {
                            Ok(Value::make_array(
                                d.borrow()
                                    .iter()
                                    .map(|(k, v)| {
                                        Value::make_array(vec![Value::Str(k.clone()), v.clone()])
                                    })
                                    .collect(),
                            ))
                        } else {
            ...
            "Set" | "set_new" => {
                ...
                for item in init_items {
                    if !unique.iter().any(|v: &Value| v.equals(&item)) {
                        unique.push(item);
                    }
                }
                let mut map = HashMap::new();
                ...
                map.insert(
                    "delete".to_string(),
                    Value::Builtin("set_delete".to_string()),
                );
                map.insert(
                    "to_array".to_string(),
                    Value::Builtin("set_to_array".to_string()),
                );
                ...
                match &args[0] {
                    Value::Object(o) => {
                        ...
                        if let Some(Value::Array(d)) = data {
                            if !d.borrow().iter().any(|v| v.equals(&args[1])) {
            ...
            "set_has" => {
                ...
                match &args[0] {
                    Value::Object(o) => {
                        let data = o.borrow().get("_data").cloned();
                        if let Some(Value::Array(d)) = data {
                            Ok(Value::Bool(d.borrow().iter().any(|v| v.equals(&args[1]))))
                        } else {
                            Ok(Value::Bool(false))
                        }
                    }
                ...
                match &args[0] {
                    Value::Object(o) => {
                        ...
                        if let Some(Value::Array(d)) = data {
                            ...
                            o.borrow_mut()
                                .insert("size".to_string(), Value::Number(after as f64));
            ...
            "set_to_array" => {
                ...
                match &args[0] {
                    Value::Object(o) => {
                        let data = o.borrow().get("_data").cloned();
                        if let Some(Value::Array(d)) = data {
                            Ok(Value::make_array(d.borrow().clone()))
                        } else {
                            Ok(Value::make_array(vec![]))
                        }
                    }
            ...
            "set_union" | "set_intersection" | "set_difference" => {
                ...
                let get_data = |o: &Value| -> Vec<Value> {
                    if let Value::Object(obj) = o {
                        if let Some(Value::Array(d)) = obj.borrow().get("_data") {
                ...
                let result = match name {
                    ...
                    "set_intersection" => a
                        .iter()
                        .filter(|v| b.iter().any(|bv| v.equals(bv)))
                        .cloned()
                        .collect(),
                    _ => a
                        .iter()
                ...
                match &args[0] {
                    Value::Object(o) => {
                        ...
                        if !is_ok || matches!(val, Value::Null) {
                            ...
                            map.insert(
                                "then".to_string(),
                                Value::Builtin("promise_then".to_string()),
                            );
                            map.insert(
                                "catch".to_string(),
                                Value::Builtin("promise_catch".to_string()),
                            );
                            map.insert(
                                "finally".to_string(),
                                Value::Builtin("promise_finally".to_string()),
                            );
                            Ok(Value::make_object(map))
                        } else {
                            Ok(args[0].clone())
            ...
            "promise_all" => {
                ...
                match &args[0] {
                    Value::Array(arr) => {
                        ...
                        for item in items {
                            let val = if let Value::Object(o) = &item {
                                o.borrow().get("value").cloned().unwrap_or(Value::Null)
                            } else {
                                item
                            };
                            results.push(val);
            ...
            "Channel" | "channel_new" => {
                ...
                map.insert(
                    "close".to_string(),
                    Value::Builtin("chan_close".to_string()),
                );
                Ok(Value::make_object(map))
            }
            "chan_send" => {
                ...
                match &args[0] {
                    ...
                    _ => Err(CustomLangError::type_err("channel.send requires Channel")),
                }
            }
            "chan_recv" => {
                if argc != 1 {
                ...
                match &args[0] {
                    Value::Object(o) => {
                        if let Some(Value::Array(d)) = o.borrow().get("_data") {
                            Ok(if d.borrow().is_empty() {
                                Value::Null
                            } else {
                                d.borrow_mut().remove(0)
                            })
                        } else {
                            Ok(Value::Null)
                        }
                    }
                    _ => Err(CustomLangError::type_err("channel.recv requires Channel")),
                }
            }
            "chan_close" => {
                if argc != 1 {
                ...
                match &args[0] {
                    Value::Object(o) => {
                        o.borrow_mut()
                            .insert("_closed".to_string(), Value::Bool(true));
                        Ok(Value::Null)
                    }
                    _ => Err(CustomLangError::type_err("channel.close requires Channel")),
                }
            }
            ...
            "actor_send" => {
                ...
                match &args[0] {
                    Value::Object(o) => {
                        ...
                        let new_state =
                            self.call_value(handler, vec![state, args[1].clone()], None, pos)?;
                ...
                match &args[0] {
                    Value::Object(o) => {
                        ...
                        if let Value::Object(_) = msg {
                            ...
                            o.borrow_mut()
                                .insert("_state".to_string(), new_state.clone());
                            return Ok(new_state);
                        }
            ...
            "sandbox_new" | "Sandbox" => {
                let opts = if argc == 1 {
                    ...
                    Value::make_object(HashMap::new())
                };
                let allow_fs = if let Value::Object(o) = &opts {
                    !matches!(o.borrow().get("allow_fs"), Some(Value::Bool(false)))
                } else {
                    true
                };
                let allow_network = if let Value::Object(o) = &opts {
                    !matches!(o.borrow().get("allow_network"), Some(Value::Bool(false)))
                } else {
                    true
                };
                let timeout = if let Value::Object(o) = &opts {
                    match o.borrow().get("timeout_ms") {
                ...
                map.insert(
                    "eval".to_string(),
                    Value::Builtin("sandbox_eval".to_string()),
                );
                Ok(Value::make_object(map))
            }
            "sandbox_eval" => {
            ...
            "csv_parse" => {
                ...
                match &args[0] {
                    Value::Str(s) => {
                        ...
                        let headers: Vec<&str> = if !lines.is_empty() {
            ...
            "priority_queue_new" | "PriorityQueue" => {
                ...
                map.insert(
                    "enqueue".to_string(),
                    Value::Builtin("pq_enqueue".to_string()),
                );
                map.insert(
                    "dequeue".to_string(),
                    Value::Builtin("pq_dequeue".to_string()),
                );
            ...
            "pq_enqueue" => {
                ...
                match &args[0] {
                    ...
                    _ => Err(CustomLangError::type_err(
                        "pq.enqueue requires PriorityQueue",
                    )),
                }
            }
            "pq_dequeue" => {
                ...
                match &args[0] {
                    ...
                    _ => Err(CustomLangError::type_err(
                        "pq.dequeue requires PriorityQueue",
                    )),
                }
            }
            "pq_peek" => {
                ...
                match &args[0] {
                    Value::Object(o) => {
                        if let Some(Value::Array(d)) = o.borrow().get("_data") {
                            Ok(d.borrow().first().cloned().unwrap_or(Value::Null))
                        } else {
                            Ok(Value::Null)
                        }
                    }
                    _ => Err(CustomLangError::type_err("pq.peek requires PriorityQueue")),
                }
            }
            ...
            "fs_list_dir_recursive" => {
                ...
                match &args[0] {
                    ...
                    _ => Err(CustomLangError::type_err(
                        "list_dir_recursive() requires string",
                    )),
                }
            }
            ...
            "proc_spawn" => {
                ...
                match &args[0] {
                    Value::Str(cmd) => {
                        let cmd_args: Vec<String> = if argc > 1 {
                            match &args[1] {
                                Value::Array(a) => {
                        ...
                        map.insert(
                            "args".to_string(),
                            Value::make_array(
                                cmd_args.iter().map(|s| Value::Str(s.clone())).collect(),
                            ),
                        );
                        map.insert(
                            "write".to_string(),
                            Value::Builtin("proc_spawn_write".to_string()),
                        );
                        map.insert(
                            "read_line".to_string(),
                            Value::Builtin("proc_spawn_read".to_string()),
                        );
                        map.insert(
                            "kill".to_string(),
                            Value::Builtin("proc_spawn_kill".to_string()),
                        );
                        Ok(Value::make_object(map))
                    }
            ...
            "named_call" => {
                ...
                match (&args[0], &args[1]) {
                    ...
                    _ => Err(CustomLangError::type_err(
                        "named_call() requires (function, object)",
                    )),
                }
            }
            ...
            "trampoline" => {
                ...
                let mut result = self.call_value(
                    args[0].clone(),
                ...
                while (matches!(&result, Value::Function(_)) || matches!(&result, Value::Object(_)))
                    && iterations < 1000000
                {
                    result = match result {
            ...
            "define_macro" => {
                ...
                match &args[0] {
                    Value::Str(name) => {
                        let name = name.clone();
                        Env::define(&self.env, &name, args[1].clone());
                        Ok(Value::Null)
                    }
                    _ => Err(CustomLangError::type_err(
                        "define_macro() requires (string, function)",
                    )),
                }
            }
    ...
    fn extract_regex_pattern(v: &Value) -> Option<String> {
        match v {
            ...
            Value::Object(o) => o.borrow().get("pattern").and_then(|p| {
...
fn parse_json(s: &str) -> std::result::Result<Value, String> {
    ...
    match s.chars().next().unwrap() {
        ...
        '[' => {
            if s == "[]" {
                return Ok(Value::make_array(vec![]));
            }
            let inner = &s[1..s.len() - 1].trim();
            let parts = split_json_array(inner);
            let vals: std::result::Result<Vec<Value>, _> =
                parts.iter().map(|p| parse_json(p.trim())).collect();
            Ok(Value::make_array(vals?))
        }
        '{' => {
            if s == "{}" {
                return Ok(Value::make_object(HashMap::new()));
            }
            let inner = &s[1..s.len() - 1].trim();
            let parts = split_json_array(inner);
            let mut map = HashMap::new();
        ...
        _ => s
            ...
            .parse::<f64>()
            .map(Value::Number)
            .map_err(|_| format!("invalid JSON: {s}")),
    }
}
    ...
    match v {
        ...
        Value::Array(arr) => {
            ...
            if let Some(ind) = indent {
                ...
                let parts: Vec<String> = items
                    .iter()
                    .map(|x| format!("{}{}", inner_indent, stringify_json(x, indent, depth + 1)))
                    .collect();
                format!("[\n{}\n{}]", parts.join(",\n"), outer_indent)
            } else {
        ...
        Value::Object(obj) => {
            ...
            if let Some(ind) = indent {
                ...
                let parts: Vec<String> = pairs
                    ...
                    .map(|(k, val)| {
                        format!(
                            ...
                            inner_indent,
                            k,
                            stringify_json(val, indent, depth + 1)
                        )
                    })
                    .collect();
                format!("{{\n{}\n{}}}", parts.join(",\n"), outer_indent)
                ...
                let parts: Vec<String> = pairs
                    .iter()
                    .map(|(k, val)| format!("\"{}\":{}", k, stringify_json(val, None, 0)))
                    .collect();
                format!("{{{}}}", parts.join(","))
            }
        }
...
fn lcg_rand(seed: u32) -> u64 {
    ...
    if s == 0 {
        s = seed as u64 ^ 6364136223846793005;
    }
    s = s
...
fn base64_encode(bytes: &[u8]) -> String {
    ...
    for chunk in bytes.chunks(3) {
        ...
        result.push(if chunk.len() > 1 {
            CHARS[(combined >> 6) & 63] as char
        } else {
            '='
        });
        result.push(if chunk.len() > 2 {
            CHARS[combined & 63] as char
        } else {
            '='
        });
    }
...
fn sha512_hex(data: &[u8]) -> String {
    let a = sha256_hash(data);
    let b = sha256_hash(&a);
    format!("{}{}", hex_encode(&a), hex_encode(&b))
}
...
fn md5_hash(data: &[u8]) -> [u8; 16] {
    ...
    let mut msg = data.to_vec();
    let orig_len = (data.len() as u64).wrapping_mul(8);
    msg.push(0x80);
    while msg.len() % 64 != 56 {
        msg.push(0);
    }
    for b in orig_len.to_le_bytes() {
        msg.push(b);
    }
    for chunk in msg.chunks(64) {
        let m: Vec<u32> = (0..16)
